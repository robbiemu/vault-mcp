...
async def test_merkle_tree_integration(integration_config, temp_vault_dir):
    ...
    service = VaultService(config=integration_config, vector_store=Mock(), query_engine=None)
    
    # Mock the document loading to return specific documents
    mock_documents = [
        Document(text="# Test Document 1\n\nThis is the first test document.", metadata={"file_path": str(file1)}),
        Document(text="# Test Document 2\n\nThis is the second test document.", metadata={"file_path": str(file2)}),
    ]
    ...
    with patch("components.vault_service.main.load_documents") as mock_load_documents:
        ...
        with patch.object(service.state_tracker, 'generate_tree_from_vault', return_value=(new_tree, new_manifest)):
...
async def test_merkle_tree_with_prefix_filtering(integration_config, temp_vault_dir):
    ...
    service = VaultService(config=integration_config, vector_store=Mock(spec=VectorStore), query_engine=None)
    
    # Mock the document loading to return specific documents
    mock_documents = [
        Document(text="# Included Document 1\n\nThis document should be included.", metadata={"file_path": str(file1)})
    ]
